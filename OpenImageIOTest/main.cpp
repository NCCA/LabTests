...
//  std::unique_ptr<unsigned char[]> pixels=std::make_unique<unsigned char []>(xres*yres*channels);
  ...
  REQUIRE(pixels[0]==Approx(255).epsilon(25));
  REQUIRE(pixels[1]==Approx(255).epsilon(25));
  REQUIRE(pixels[2]==Approx(255).epsilon(25));

  size_t index           = (2 * WIDTH * 3) + 2 * 3;
  // red
  REQUIRE(pixels[index]==Approx(255).epsilon(25));
  REQUIRE(pixels[index+1]==Approx(0).epsilon(25));
  REQUIRE(pixels[index+2]==Approx(0).epsilon(25));
